...
class Client {
    ...
    public $adult = true; // default false in API
    public $language = 'en-US'; // ISO 639-1: default already in API
    public $region = 'AU'; // ISO 3166-1: filters release dates for this region
    public $paged = true;
    public $error;
    ...
    private function send_request($method, $params = array() , $data = array()) {
        ...
        if ($method!='configuration') {
            //$url = $this->api_url . '/' . $this->api_version . '/' . 'discover/movie?api_key=dd7b7542fb90a25aea16ecb23f8a6217&include_adult=1&language=en-US&sort_by=vote_average.desc,popularity.desc&include_video=false&certification_country=US&page=1';
            //die($url);
        }
